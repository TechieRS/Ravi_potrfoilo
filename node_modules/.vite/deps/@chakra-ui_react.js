...
function parseToRgba(color2) {
  if (typeof color2 !== "string") throw new ColorError(color2);
  ...
  if (hslaMatch) {
    ...
    if (guard(0, 100, s) !== s) throw new ColorError(color2);
    if (guard(0, 100, l) !== l) throw new ColorError(color2);
    return [...hslToRgb(h, s, l), Number.isNaN(a) ? 1 : a];
  }
  throw new ColorError(color2);
}
function hash(str) {
  let hash5 = 5381;
  let i = str.length;
...
function nameToHex(color2) {
  ...
  if (!result) throw new ColorError(color2);
  return `#${result}`;
}
